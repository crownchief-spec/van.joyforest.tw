...
def record_centroid(shape):
    points = visible_main_part(shape.points)
    if not points:
        return None
    return project((sum(p[0] for p in points) / len(points), sum(p[1] for p in points) / len(points)))


def draw_label(draw, xy, zh, en, target, left_side):
    x, y = xy
    w, h = 460, 70
    draw.rounded_rectangle((x, y, x + w, y + h), 15, fill=WHITE, outline="#DED6C8", width=2)
    draw.text((x + 18, y + 8), zh, font=font(ZH_FONT, 24), fill=INK)
    draw.text((x + 18, y + 38), en, font=font(EN_FONT, 18), fill=MUTED)
    start = (x + w, y + h // 2) if left_side else (x, y + h // 2)
    elbow_x = 575 if left_side else 1225
...
def draw_inset(draw, title_zh, title_en, box, shapes):
    x1, y1, x2, y2 = box
    draw.rounded_rectangle(box, 18, fill=WHITE, outline="#DED6C8", width=2)
    draw.text((x1 + 18, y1 + 12), title_zh, font=font(ZH_FONT, 23), fill=INK)
    draw.text((x1 + 18, y1 + 43), title_en, font=font(EN_FONT, 17), fill=MUTED)
    ...
    pad = 20
    map_box = (x1 + pad, y1 + 78, x2 - pad, y2 - pad)
